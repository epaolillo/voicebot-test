...
int main(int argc, char *argv[])
{
    ...
    fprintf(stderr, "[config] company=%s  caller=%s %s  autolistening=%d\n",
    ...
    while (running) {
        ...
        if (speech_active && backchannel_armed && state == VAD_SPEECH) {
        ...
        if (state == VAD_SPEECH_END && !raw_recording.empty()) {
            ...
            if (wav_data) {
                ...
                pthread_create(&whisper_thread, nullptr, [](void *arg) -> void * {
                    ...
                    j->result = whisper_api_transcribe(j->wav, j->size, &j->text);
                    timer_stop(&j->timer, "Whisper STT");
                    return nullptr;
                }, &wjob);

                /* Meanwhile: delay + filler TTS (runs in parallel with Whisper) */
                ...
                if (wjob.result == 0 && wjob.text) {
                    ...
                    if (strlen(wjob.text) > 0) {
                        ...
                        timer_stop(&phase_timer, "LLM + TTS (total)");
